...
def get_input_data_tensors(reader,
  ...
  with tf.name_scope("train_input"):
    files = gfile.Glob('gs://youtube8m-ml-us-east1/1/video_level/train/train*.tfrecord') + gfile.Glob('gs://youtube8m-ml-us-east1/1/video_level/validate/validate*.tfrecord')
    if not files:
...
def build_graph(reader,
  ...
  tower_gradients = []
  tower_init_gradients = []
  tower_predictions = []
  tower_init_predictions = []
  tower_label_losses = []
  tower_init_label_losses = []
  tower_reg_losses = []
    ...
    with tf.device(device_string % i):
      with (tf.variable_scope(("tower"), reuse=True if i > 0 else None)):
        with (slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0" if num_gpus!=1 else "/gpu:0")):
          ...
          init_predictions = result["init_predictions"]
          tower_init_predictions.append(init_predictions)

          if "loss" in result.keys():
            label_loss = result["loss"]
          else:
            label_loss = label_loss_fn.calculate_loss(predictions, tower_labels[i])
            init_label_loss = losses.init_CrossEntropyLoss().calculate_loss(init_predictions, tower_labels[i])
          ...
          if update_ops:
            with tf.control_dependencies(update_ops):
              ...
              with tf.control_dependencies([barrier]):
                ...
                init_label_loss = tf.identity(init_label_loss)

          tower_label_losses.append(label_loss)
          tower_init_label_losses.append(init_label_loss)

          # Incorporate the L2 weight penalties etc.
          final_loss = regularization_penalty * reg_loss + label_loss
          init_final_loss = regularization_penalty * reg_loss + init_label_loss
          gradients = optimizer.compute_gradients(final_loss,
              colocate_gradients_with_ops=False)
          init_gradients = optimizer.compute_gradients(init_final_loss,
              colocate_gradients_with_ops=False)
          tower_gradients.append(gradients)
          tower_init_gradients.append(init_gradients)
  label_loss = tf.reduce_mean(tf.stack(tower_label_losses))
  init_label_loss = tf.reduce_mean(tf.stack(tower_init_label_losses))
  if regularization_penalty != 0:
    reg_loss = tf.reduce_mean(tf.stack(tower_reg_losses))
  merged_gradients = utils.combine_gradients(tower_gradients)
  init_merged_gradients = utils.combine_gradients(tower_init_gradients)

  if clip_gradient_norm > 0:
    with tf.name_scope('clip_grads'):
      merged_gradients = utils.clip_gradient_norms(merged_gradients, clip_gradient_norm)
      init_merged_gradients = utils.clip_gradient_norms(init_merged_gradients, clip_gradient_norm)

  train_op = optimizer.apply_gradients(merged_gradients, global_step=global_step)
  init_train_op = optimizer.apply_gradients(init_merged_gradients, global_step=global_step)
  ...
  tf.add_to_collection("init_train_op", init_train_op)
  ...
  def run(self, start_new_model=False):
    ...
    with tf.Graph().as_default() as graph:
      ...
      with tf.device(device_fn):
        ...
        init_train_op = tf.get_collection("init_train_op")[0]
